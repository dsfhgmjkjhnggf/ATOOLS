...
--
local Lines_ClrStdAlph = new.int(255)
local Lines_ClrStat = new.float[4](1.0, 1.0, 1.0, 1.0)
local Lines_ClrDynAlph = new.int(255)
local Lines_ClrDynSpeed = new.float(2)
local Lines_ClrDynShift = new.bool(true)
--
...
--
local Bones_ClrStdAlph = new.int(255)
local Bones_ClrStat = new.float[4](1.0, 1.0, 1.0, 1.0)
local Bones_ClrDynAlph = new.int(255)
local Bones_ClrDynSpeed = new.float(2)
local Bones_ClrDynShift = new.bool(true)
--
local Bones_Thickness = new.float(1)
local Bones_VisDistance = new.int(300)

--------- Концы костей
local BoneEnds_Active = new.bool(true)
--
local BoneEnds_ClrType = new.int(1)
...
--
local BoneEnds_ClrStdAlph = new.int(255)
local BoneEnds_ClrStat = new.float[4](1.0, 1.0, 1.0, 1.0)
local BoneEnds_ClrDynAlph = new.int(255)
local BoneEnds_ClrDynSpeed = new.float(2)
local BoneEnds_ClrDynShift = new.bool(true) 
--
...
local Tag_VisWall = new.bool(true)
local Tag_VisDistance = new.float(300)
local Tag_VisWallInt = 0
...
local newFrame = imgui.OnFrame(function() return wMain[0] end, function(player)
    ...
    imgui.BeginChild('Name', imgui.ImVec2(525, 300), true)
        ...
        if imgui.BeginPopup('ESP_Bones') then
            ...
            imgui.DragInt(u8'Дистанция отрисовки##Bones_VisDistance', Bones_VisDistance, 1, 1, 1000) imgui.NewLine()
            imgui.Text(u8"Настройки костей:") imgui.Separator()
            imgui.PushItemWidth(135) imgui.Combo(u8"Режим цвета##Bones_ClrType", Bones_ClrType, Bones_ClrTypeItems, #Bones_ClrTypeArray) imgui.PopItemWidth() imgui.SameLine()
            if imgui.Button(u8"Sync с костями") then
                BoneEnds_ClrType[0] = Bones_ClrType[0]
                if Bones_ClrType[0] == 0 then BoneEnds_ClrStdAlph[0] = Bones_ClrStdAlph[0]
                elseif Bones_ClrType[0] == 1 then for i = 0, 3 do BoneEnds_ClrStat[i] = Bones_ClrStat[i] end
                elseif Bones_ClrType[0] == 2 then
                    BoneEnds_ClrDynAlph[0] = Bones_ClrDynAlph[0]
                    BoneEnds_ClrDynSpeed[0] = Bones_ClrDynSpeed[0]
                    BoneEnds_ClrDynShift[0] = Bones_ClrDynShift[0]
                end
            end
            if Bones_ClrType[0] == 0 then imgui.DragInt(u8'Прозрачность##Bones_ClrStdAlph', Bones_ClrStdAlph, 0, 0, 255)
            elseif Bones_ClrType[0] == 1 then imgui.ColorEdit4(u8("Цвет##Bones_ClrStat"), Bones_ClrStat, imgui.ColorEditFlags.NoInputs)
            elseif Bones_ClrType[0] == 2 then
                imgui.Checkbox(u8'Сдвиг палитры по ID##Bones_ClrDynShift', Bones_ClrDynShift)
                imgui.SameLine()
                imgui.DragInt(u8'Прозрачность##Bones_ClrDynAlph', Bones_ClrDynAlph, 0, 0, 255)
                imgui.SameLine()
                imgui.DragFloat(u8'Скорость##Bones_ClrDynSpeed', Bones_ClrDynSpeed, 0, 0, 10, "%.1f") end
            imgui.Separator()
            imgui.DragFloat(u8'Толщина##Bones_Thickness', Bones_Thickness, 0, 1, 3, "%.1f")
            imgui.NewLine()
            imgui.Text(u8"Настройки конца костей:")
            imgui.Checkbox(u8'Концы костей', BoneEnds_Active)
            imgui.Separator()
            imgui.PushItemWidth(135)
            imgui.Combo(u8"Режим цвета##BoneEnds_ClrType", BoneEnds_ClrType, BoneEnds_ClrTypeItems, #BoneEnds_ClrTypeArray)
            imgui.PopItemWidth()
            if BoneEnds_ClrType[0] == 0 then
                imgui.DragInt(u8'Прозрачность##BoneEnds_ClrStdAlph', BoneEnds_ClrStdAlph, 0, 0, 255)
            elseif BoneEnds_ClrType[0] == 1 then
                imgui.ColorEdit4(u8("Цвет##BoneEnds_ClrStat"), BoneEnds_ClrStat, imgui.ColorEditFlags.NoInputs)
            elseif BoneEnds_ClrType[0] == 2 then
                imgui.Checkbox(u8'Сдвиг палитры по ID##BoneEnds_ClrDynShift', BoneEnds_ClrDynShift)
                imgui.SameLine()
                imgui.DragInt(u8'Прозрачность##BoneEnds_ClrDynAlph', BoneEnds_ClrDynAlph, 0, 0, 255)
                imgui.SameLine()
                imgui.DragFloat(u8'Скорость##BoneEnds_ClrDynSpeed', BoneEnds_ClrDynSpeed, 0, 0, 10, "%.1f")
            end
            imgui.Separator()
            imgui.PushItemWidth(110) imgui.Combo(u8"Фигура", BoneEnds_Figure, BoneEnds_FigureItems, #BoneEnds_FigureArray) imgui.PopItemWidth()
            imgui.SameLine()
        ...
        if imgui.BeginPopup('ESP_Lines') then
            ...
            imgui.PushItemWidth(135) imgui.Combo(u8"Режим цвета##Lines_ClrType", Lines_ClrType, Lines_ClrTypeItems, #Lines_ClrTypeArray) imgui.PopItemWidth() imgui.SameLine()
            if imgui.Button(u8"Sync от костей") then
                cfg()
                Lines_ClrType[0] = Bones_ClrType[0]
                if Bones_ClrType[0] == 0 then Lines_ClrStdAlph[0] = Bones_ClrStdAlph[0]
                elseif Bones_ClrType[0] == 1 then for i = 0, 3 do Lines_ClrStat[i] = Bones_ClrStat[i] end
                elseif Bones_ClrType[0] == 2 then
                    Lines_ClrDynAlph[0] = Bones_ClrDynAlph[0]
                    Lines_ClrDynSpeed[0] = Bones_ClrDynSpeed[0]
                    Lines_ClrDynShift[0] = Bones_ClrDynShift[0]
                end
            end
            imgui.PushItemWidth(35)
            if Lines_ClrType[0] == 0 then imgui.DragInt(u8'Прозрачность##Lines_ClrStdAlph', Lines_ClrStdAlph, 0, 0, 255)
            elseif Lines_ClrType[0] == 1 then imgui.ColorEdit4(u8("Цвет##Lines_ClrStat"), Lines_ClrStat, imgui.ColorEditFlags.NoInputs)
            elseif Lines_ClrType[0] == 2 then
                imgui.Checkbox(u8'Сдвиг палитры по ID##Bones_ClrDynShift', Lines_ClrDynShift) imgui.SameLine()
                imgui.DragInt(u8'Прозрачность##Bones_ClrDynAlph', Lines_ClrDynAlph, 0, 0, 255) imgui.SameLine()
                imgui.DragFloat(u8'Скорость##Bones_ClrDynSpeed', Lines_ClrDynSpeed, 0, 0, 10, "%.1f") end
            imgui.Separator()
        ...
        if imgui.BeginPopup('ESP_Tags') then
            ...
            if imgui.Checkbox(u8'Видимость через стены', Tag_VisWall) and Tag_Active[0] then nameTagON() end
            imgui.PushItemWidth(35)
            if imgui.DragFloat(u8'Дальность прорисовки ников##Tag_VisDistance', Tag_VisDistance, 1, 40, 1000, "%.1f") and Tag_Active[0] then nameTagON() end
            imgui.PopItemWidth()
            imgui.EndPopup()
        end      
    imgui.EndChild()
        ...
        if sampIsPlayerConnected(i) then -- Проверка подключён ли игрок
            ...
            if find and doesCharExist(handle) and isCharOnScreen(handle)  then -- Игрок найден | персонаж существует | персонаж в зоне видимости
                ...
                if Lines_Active[0] and distance < Lines_VisibilityDistance[0] then
                    local color
                    if Lines_ClrType[0] == 0 then
                        color = join_argb(Lines_ClrStdAlph[0], rr, gg, bb)
                    elseif Bones_ClrType[0] == 1 then
                        color = join_rgba(Lines_ClrStat[0], Lines_ClrStat[1], Lines_ClrStat[2], Lines_ClrStat[3])
                    elseif Lines_ClrType[0] == 2 then
                        if Lines_ClrDynShift[0] then color = rainbow(Lines_ClrDynSpeed[0], Lines_ClrDynAlph[0], i*20) else color = rainbow(Lines_ClrDynSpeed[0], Lines_ClrDynAlph[0]) end
                    end
                    ...
                    renderDrawLine(myPosScreen[1], myPosScreen[2], enPosScreen[1], enPosScreen[2], Lines_Thickness[0], color) -- Отрисовываем
                end

                -- Кости
                if Bones_Active[0] and distance < Bones_VisDistance[0] then
                    local color
                    if Bones_ClrType[0] == 0 then
                        color = join_argb(Bones_ClrStdAlph[0], rr, gg, bb)
                    elseif Bones_ClrType[0] == 1 then
                        color = join_rgba(Bones_ClrStat[0], Bones_ClrStat[1], Bones_ClrStat[2], Bones_ClrStat[3])
                    elseif Bones_ClrType[0] == 2 then
                        if Bones_ClrDynShift[0] then color = rainbow(Bones_ClrDynSpeed[0], Bones_ClrDynAlph[0], i*20) else color = rainbow(Bones_ClrDynSpeed[0], Bones_ClrDynAlph[0]) end
                    end
                    ...
                    if BoneEnds_Active[0] then
                        local color
                        if BoneEnds_ClrType[0] == 0 then
                            color = join_argb(BoneEnds_ClrStdAlph[0], rr, gg, bb)
                        elseif BoneEnds_ClrType[0] == 1 then
                            color = join_rgba(BoneEnds_ClrStat[0], BoneEnds_ClrStat[1], BoneEnds_ClrStat[2], BoneEnds_ClrStat[3])
                        elseif BoneEnds_ClrType[0] == 2 then
                            if BoneEnds_ClrDynShift[0] then color = rainbow(BoneEnds_ClrDynSpeed[0], BoneEnds_ClrDynAlph[0], i*20) else color = rainbow(BoneEnds_ClrDynSpeed[0], BoneEnds_ClrDynAlph[0]) end
                        end
                        ...
                        for v = 1, #t do
                            ...
                            local size = BoneEnds_Size[0] / math.sqrt(distance / 2)
...
function nameTagON()
    ...
    if Tag_VisWall[0] then Tag_VisWallInt = 0 else Tag_VisWallInt = 1 end
	mem.setfloat(pStSet + 39, Tag_VisDistance[0])
